...
for rate in 1000 2000 3000 4000 5000; do
    ...
    sleep 1
    POSTGRES="host=localhost port=6432 user=$USER password=$PGPASSWORD dbname=$USER" ./hasql-loadtest-template/bin/testing-service -p 9000 > logs/$TEST_NAME-server-9000-"$PROFILE_TAG".log 2> logs/$TEST_NAME-server-9000-"$PROFILE_TAG".err & write_pid
    ...
    # Run main tester
    ./wrk2/wrk -d 60 -t 2 -c 2000 --rate 2000 "http://localhost:9000/plain" | tee logs/$TEST_NAME-wrk2-"${PROFILE_TAG}".log
