...
for rate in 1000 2000 3000 4000 5000 6000 7000; do
    ...
    ./odyssey/build/sources/odyssey ./odyssey.conf > logs/$TEST_NAME-pooler-$PROFILE_TAG.log 2> logs/$TEST_NAME-pooler-$PROFILE_TAG.err & echo $! >> var/pid
    sleep 1
    POSTGRES="host=localhost port=6432 user=$USER password=$PGPASSWORD dbname=$USER" ./hasql-loadtest-template/bin/testing-service -p 9001 > logs/$TEST_NAME-server-9001-$PROFILE_TAG.log 2> logs/$TEST_NAME-server-9001-$PROFILE_TAG.err & echo $! >> var/pid
